...
Before proceeding, let's check whether our GBIF occurrence dataset shows any signs of bias by analyzing how the density of occurrences varies across the study area. We can do this by counting the number of observations within each grid cell. To accomplish this, we will use the [r.vect.stats](https://grass.osgeo.org/grass-stable/manuals/addons/r.vect.stats.html) module, which allows us to perform statistical analysis on raster-vector combinations. Since this is an addon, we will need to install it first.

::: {#exm-upfOO5Bwjm .hiddendiv}
:::
...
In the menu, select [Settings → Addon extensions → Install extension from addons \[g.extension\]]{.style-menu}. This opens a new window (see below).

![Use the search bar to look for [r.vect.stats]{.style-function} and hit [Enter]{.style-menu}](images/g_extension_install_rvectstats.png){#fig-installrvectstats fig-align="left" width="80%"}
:::

We want to create the layer in the [Erebia_albergana]{.style-db} mapset, so we need to switch to that mapset first.
...
The extent and resolution of the resulting raster is determined by the bounding box of the mapset. So we set the region's extent to match the extent of the occurrence point layer. We also set the region's resolution to match that of the bioclimate layers, which is 0.008333 arc degrees.
...
``` bash
g.region -a res=0.008333 vector=Erebia_albergana_obs # <1>
```
...
``` bash
r.vect.stats input=Erebia_albergana_obs output=pointdensities method=n
```
...
We use [r.univar](https://grass.osgeo.org/grass-stable/manuals/r.univar.html) to compute the range and median of number of observations per gridd cell and the [r.boxplot](https://grass.osgeo.org/grass-stable/manuals/addons/r.boxplot.html) addon to visualize the distribution of point densities and to identify possible outliers.
...
We'll use all land areas within the area of interest (aoi) that we defined in @sec-climatedata. To keep organized, we'll create a new mapset [aoi_01]{.style-db} to keep the dataset belonging to this particular *aoi*. We'll the [g.mapset](https://grass.osgeo.org/grass-stable/manuals/g.mapset.html) module.

Next, we then use the [g.region](https://grass.osgeo.org/grass-stable/manuals/g.region.html) module to set the computational region style parameter, based on the region file we created earlier (@exm-V0FK3TyzcN). We use the [r.mask](https://grass.osgeo.org/grass-stable/manuals/r.mask.html) module with the [vector]{.style-parameter} parameter to further restrict our study area to land areas. And finally, to avoid having to append the name of that mapset to a layers name, each time we need a layer from a different mapset, we give access to the other relevant mapsets using the [g.mapsets](https://grass.osgeo.org/grass-stable/manuals/g.mapsets.html) command [^3_data_preparation-2].
...
g.mapsets mapset=climate_current,Erebia_albergana operation=add
```
...
gs.run_command("g.mapset", flags="c", mapset="aoi_01")
...
In the menu, go to [Settings → GRASS working environment → Mapset access]{.style-menu} and select the mapsets [climate_current]{.style-db} and [Erebia_albergana]{.style-db}.
:::

We have now a new raster layer named [MASK]{.style-data} in the mapset [aoi_01]{.style-db}. We can use [r.info](https://grass.osgeo.org/grass-stable/manuals/r.info.html) to check that it has the same extent and resolution as the computational region ([@fig-studyareas]). The raster cells falling outside the MASK are not included in operations like raster algebra, statistics, or interpolation.
...
![The bioclim 6 and bioclim 11 variables are highly correlated (r=0.98). This begs the question how useful it is to use both as predictor variables in the model. You can explore the relation between any two variables yourself using the bivariate scatterplot tool. Click on the ![](images/layer-raster-analyze.png) in the toolbar of the map display.](images/bio6_bio11_scatterplot.png){#fig-bio6_bio11_scatterplot fig-align="left"}
...
g.extension extension=r.vif # <1>

# Select the input variable
layers=`g.list -m type=raster pattern=bio* sep="," mapset=climate_current`  # <2>
r.vif maps=$layers seed=5 n=100000 maxvif=10 # <3>
```

1.  Installs the r.vif add-on in GRASS GIS if it is not already installed.
2.  This line creates a comma separated list of raster layers in the mapset 'climate_current' whose names start with 'bio'. The resulting list is stored in the variable 'layers'. With the *-m* flag, the mapset is included in the layers names.
3.  **maps=\$layers**: Uses the list of raster layers stored in the 'layers' variable as input for the VIF analysis. You can, alternatively, skip the previous line, and type in the names of each of the layers.

## {{< fa brands python >}}

``` python
# Install the r.vif addon
gs.run_command("g.extension", extension="r.vif")  # <1>

# Select the input variable
layers = gs.list_strings(pattern="bio*", mapset="climate_current", type="raster")  # <2>
gs.run_command("r.vif", maps=layers, seed=5, n=100000, maxvif=10) # <3>
```

1.  Installs the r.vif add-on in GRASS GIS if it is not already installed.
2.  The [list_strings]{.style-function} module provides a convenient wrapper to the [g.list]{.style-function}. The parameters are the same as for the [g.list]{.style-function}.
3.  The [seed]{.style-parameter} parameter ensures that the random processes are reproducible. You can set any number.

## {{< fa regular window-restore >}}

In the menu, go to [settings → addon extensions → install extensions from addons]{.style-menu}, search for [r.vif]{.style-function} and install it.

Now, we run the [r.vif]{.style-function}. Make sure to fill in the names of all 19 bioclim variables under the [maps]{.style-parameter} parameter.
...
The VIF algorithm offers a data-driven method for selecting variables. However, it's important to also consider ecological significance when making selections. For instance, if a species is sensitive to temperatures during the coldest quarter, this variable should be kept regardless of its VIF score. The [r.vif]{.style-function} module includes a [retain]{.style-parameter} parameter that allows you to specify one or more variables to retain during the stepwise selection process. If a retained variable has the highest VIF, the variable with the next highest VIF will be removed instead.
:::
...
v.maxent.swd -t species=Erebia_albergana_obs \                             # <1>
  bgp=background_points \                                          # <2> 
  evp_maps=bio_1,bio_2,bio_4,bio_8,bio_9,bio_13,bio_14,bio_15 \   # <3>
...
6.  The location of the [environmental raster layers]{.style-data}. Note that if you save them to a sub-folder of your working directory, you can use the relative path.
...
gs.run_command(
    "v.maxent.swd",
    flags="t",
    species="Erebia_albergana_obs",                                          # <2>
    bgp="background_points",                                         # <3>
    evp_maps="bio_1,bio_2,bio_4,bio_8,bio_9,bio_13,bio_14,bio_15",  # <4>
...
7.  The location of the [environmental raster layers]{.style-data}. Note that if you save them to a sub-folder of your working directory, you can use the relative path.

## {{< fa regular window-restore >}}

Create the folder [dataset01]{.style-db} and sub-folder [envdat]{.style-db} using your favorite file browser. Next, open the [v.maxent.swd]{.style-function} dialog and run it with the following parameter settings:

| Parameter | Value |
|----|----|
| species [^3_data_preparation-3] | Erebia_albergana_obs |
...
[^3_data_preparation-8]: [export_rasters]{.style-parameter}: The location of the [environmental raster layers]{.style-data}. Note that if you save them to a sub-folder of your working directory, you can use the relative path.
...
species_vector = "Erebia_albergana_obs"
aoi_mapset = "aoi_01"
resolution = 0.008333
...
# Define output variables
swd_species_output = "dataset01/Erebia_albergana_obs.swd"
...
gs.run_command("g.region", region=f"{aoi_mapset}@{climate_mapset}")
