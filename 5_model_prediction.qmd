...
The minimum input requirements of [r.maxent.predict]{.style-function} are the names of the input raster layers, the [lambdas]{.style-output} file and the raster layers for the predictor variables described in the lambdas file. The [lambdas]{.style-output} file was one of the outputs from [r.maxent.train]{.style-function} and can be found in the output folder [model_01]{.style-output}. It has the extension *.lambdas* and describes the Maxent model.
