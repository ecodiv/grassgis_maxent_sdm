...
[^4_model_training-4]: The (relative) path to the folder that holds the ascii raster layers of the input environmental variables. If you select these, Maxent will first create the model, and than use the ascii layers as input in the model to create a prediction raster layer. See also point 9.

[^4_model_training-5]: The (relative) path to the folder that where Maxent should write the results.
...
[^4_model_training-12]: Create a vector point layer with predicted probability scores at the background point locations. This is useful to identify discrepancies between the observed and predicted distribution.
