...
Throughout the text you'll find literature references and footnotes. You are encouraged to check them out. Simply hoover over the link with your mouse. You'll also find that some code includes explanatory notes. You can read these notes by clicking on the number next to the code.
...
For the species distribution modeling, we'll use a two addons, [r.maxent.train](https://grass.osgeo.org/grass-stable/manuals/addons/r.maxent.train.html) and [r.maxent.predict](https://grass.osgeo.org/grass-stable/manuals/addons/r.maxent.predict.html), which provide a convenient interface to the Maxent software. Download the software from the [Maxent website](https://biodiversityinformatics.amnh.org/open_source/maxent/) and extract the [Maxent.jar]{.style-apps} file. Install it into the GRASS GIS add-on directory using the [r.maxent.setup](https://grass.osgeo.org/grass-stable/manuals/addons/r.maxent.setup.html) module. This will allow the other add-ons to find and use it automatically.
...
If you first copy the [Maxent.jar]{.style-file} file to the working directory, you only need to provide the file name as input in the code below. Otherwise, provide the full path to the file.
...
Note that you only have to do this once. If you already installed the Maxent.jar file, but want to update it, you can use the [u]{.style-parameter}. This will replace the older version with the new one.

### Java OpenJDK

The [Maxent]{.style-apps} software requires [Java]{.style-apps} or the [Java Runtime Environment]{.style-apps} (JRE) version 1.4 or later to be installed on your computer. There is a good change you already have [Java]{.style-apps} installed. You can check this using the step below.
...
Now we need to ensures that [GRASS GIS]{.style-apps} can locate the [Java]{.style-apps} executable, a prerequisite for running [Maxent]{.style-apps}. On Linux, and in some cases on Windows, GRASS may have access to [Java]{.style-apps} without further steps. You can check this using the following command.
